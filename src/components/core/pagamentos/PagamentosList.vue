...
import PagamentosListItem from './PagamentosListItem.vue';

const props = defineProps(['data'])
</script>

<template>
  <div v-if="data.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 w-full">
    <PagamentosListItem v-for="item in data" :key="item.id" :item="item" />
  </div>
