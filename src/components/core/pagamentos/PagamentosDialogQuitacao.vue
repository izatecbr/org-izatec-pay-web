...
const validateAndSubmitForm = async () => {
    ...
    const { success, status } = await pagamentos.quitacaoManual(Utils.clone(form.value), props.item.id);
            ...
            <AlertDialogHeader>
                <AlertDialogTitle class="text-start font-semibold">
                    <Badge variant="outline" class="text-sm">
                        Quitação Manual<span class="mx-1.5">|</span><strong>{{ item?.mensagem }}</strong>
                    </Badge>
